...
const RepoSelector: React.FC<RepoSelectorProps> = ({ onStartIngestion, compact = false, org, installationId }) => {
  ...
  const fetchRepos = async (attempt = 0, isRetry = false) => {
    ...
    try {
      ...
      if (err.status === 401) errorMessage = 'Authentication required. Please log in again.';
      else if (err.status === 400) errorMessage = 'Github App installation not found. Please install the app first.';
      else if (err.status === 503) errorMessage = 'Service temporarily unavailable. Retrying...';
