...
const envSchema = z.object({
  ...
  NODE_ENV: z
    ...
    .default('development')
    .describe(
      "The environment we're running in - affects logging, error handling, etc.",
    ),
  ...
  MONGODB_URI: z
    ...
    .describe(
      'Connection string for MongoDB - where we store user data and conversations',
    ),
  ...
  QDRANT_URL: z
    ...
    .describe(
      'URL for Qdrant vector database - stores code embeddings for AI search',
    ),
  ...
  QDRANT_API_KEY: z
    ...
    .optional()
    .describe(
      'Optional API key for Qdrant (if your instance requires authentication)',
    ),
  ...
  REDIS_URL: z
    ...
    .describe(
      'Connection string for Redis - powers our background job processing',
    ),
  ...
  GITHUB_APP_SLUG: z
    ...
    .describe('Your GitHub App slug/name (used for install URLs)'),
  ...
  GITHUB_REDIRECT_URI: z
    ...
    .describe(
      'Where GitHub should redirect users after OAuth (must match GitHub App settings)',
    ),

  GITHUB_APP_PRIVATE_KEY: z
    .string()
    .min(1, 'GitHub App Private Key is required')
    // Many platforms store PEM newlines as literal "\n" sequences.
    // Normalize to real newlines so crypto libraries can use it.
    .transform((key) => key.replace(/\\n/g, '\n'))
    .describe(
      'GitHub App private key (PEM format) for authenticating as the app',
    ),

  REDIRECT_AFTER_INSTALL: z
    .string()
    .url('REDIRECT_AFTER_INSTALL must be a valid URL')
    .optional()
    .describe('Optional URL to redirect to after GitHub App install'),
  ...
  COHERE_API_KEY: z
    ...
    .optional()
    .describe(
      'Optional Cohere API key for reranking search results (improves accuracy)',
    ),
  ...
  // Internal / team operations (optional)
  INTERNAL_TEAM_TOKEN: z
    .string()
    .optional()
    .describe('Optional bearer token for internal/team operations'),
  FINE_TUNING_TOKEN: z
    .string()
    .optional()
    .describe('Optional bearer token for fine-tuning/training operations'),

  // Developer / test utilities (optional)
  GITHUB_PAT: z
    .string()
    .optional()
    .describe(
      'Optional GitHub personal access token used by local test scripts',
    ),
  GITHUB_ACCESS_TOKEN: z
    .string()
    .optional()
    .describe('Optional GitHub token used by local test scripts'),
});
...
function validateEnv() {
  try {
    ...
    if (error instanceof z.ZodError) {
      ...
      console.error(
        'It looks like some environment variables are missing or invalid:\n',
      );
      ...
      console.error(
        '   You can find an example .env file in the project documentation.\n',
      );
