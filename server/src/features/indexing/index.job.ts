...
function scheduleWorkerSleep(): void {
  ...
  sleepTimer = setTimeout(() => {
    ...
    if (w) {
      w.close().then(() =>
        console.log('😴 Worker sleeping (no jobs for 15 min)'),
      );
    }
  }, IDLE_SLEEP_MS);
}
        ...
        async (job: Job<{ repoUrl: string; sha: string }>) => {
          ...
          try {
            ...
            try {
              ...
              console.error(
                '❌ Failed to update progress to 30%:',
                progressError,
              );
              throw new Error(
                `Progress update failed: ${progressError.message}`,
              );
            }
            ...
            try {
              ...
              throw new Error(
                `Failed to chunk documents: ${chunkError.message}`,
              );
            }
