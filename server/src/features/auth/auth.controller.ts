...
export const completeAuth = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const installationId = installs.installations?.find((i: any) => i.app_id === APP_ID || i.app_slug === APP_SLUG)?.id;
...
export const listRepos = async (req: Request, res: Response): Promise<void> => {
  ...
  if (!repoRes.ok) {
    console.error('GitHub /installation/repositories error', repoRes.status, repoJson);
  ...
  const repoMeta = await Promise.all(
    repositories.map(async (r: any) => {
      ...
      const headRes = await fetch(`https://api.github.com/repos/${r.full_name}/commits/${r.default_branch}`, {
        headers: {
