    ...
    res.status(429).json({
      ...
      retryAfter: Math.ceil(
        (req as any).rateLimit?.resetTime
          ...
          : 15 * 60,
      ), // Tell them how many seconds to wait
    });
  },
});
...
export const authLimiter = rateLimit({
  ...
  handler: (req: Request, res: Response) => {
    res.status(429).json({
      ...
      message:
        'Please wait before trying to authenticate again. This helps protect your account from brute force attacks.',
      retryAfter: Math.ceil(
        (req as any).rateLimit?.resetTime
          ? ((req as any).rateLimit.resetTime - Date.now()) / 1000
          : 15 * 60,
...
export const queryLimiter = rateLimit({
  ...
  handler: (req: Request, res: Response) => {
    res.status(429).json({
      ...
      message:
        "You're making queries too quickly. Please slow down to ensure quality responses for everyone.",
      retryAfter: Math.ceil(
        (req as any).rateLimit?.resetTime
          ? ((req as any).rateLimit.resetTime - Date.now()) / 1000
          : 60,
