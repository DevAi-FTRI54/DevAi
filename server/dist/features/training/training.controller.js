...
export const exportTrainingData = async (req, res) => {
    try {
        ...
        logger.info(`✅ Exported ${result.trainingPairs.length} training pairs for fine-tuning`, { uniqueUsers: uniqueUsers.length, uniqueRepos: uniqueRepos.length });
        res.json(exportData);
    }
    catch (error) {
...
export const completeTrainingRun = async (req, res) => {
    try {
        ...
        await trainingRun.save();
        logger.info(`${success ? '✅' : '❌'} Training run ${success ? 'completed' : 'failed'}`, { runId, durationMinutes: duration });
        res.json({
            runId,
