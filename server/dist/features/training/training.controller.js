...
const extractTrainingPairs = async (options) => {
    const { countOnly = false, minPairs = 200, lastDataTimestamp, userId, } = options;
    ...
    const filteredConversations = lastDataTimestamp
        ? conversations.filter((conv) => conv.messages.some((msg) => msg.timestamp > lastDataTimestamp))
        : conversations;
    // Extract all messages from all conversations
    const allMessages = filteredConversations.flatMap((conv) => conv.messages
        .filter((msg) => !lastDataTimestamp || msg.timestamp > lastDataTimestamp)
        .map((msg) => ({
        role: msg.role,
        content: msg.content,
        ...
        if (currentMsg.role === 'user' &&
            ...
            if (!countOnly) {
                ...
                const contextMessages = allMessages
                    .filter((msg) => msg.sessionId === currentMsg.sessionId &&
                ...
                const citationsText = nextMsg.citations?.length
                    ? `\n\nReferences:\n${nextMsg.citations
                        .map((c) => `- ${c.file} (lines ${c.startLine}-${c.endLine}): ${c.snippet.slice(0, 100)}...`)
                        .join('\n')}`
                    : '';
                trainingPairs.push({
                    instruction,
 ...
 */
export const checkTrainingReadiness = async (req, res) => {
    try {
        const minPairs = parseInt(req.query.min_pairs) || 200;
...
export const exportTrainingData = async (req, res) => {
    try {
        const minPairs = parseInt(req.query.min_pairs) || 200;
        const userId = req.query.user_id; // Optional: for user-specific export
        console.log(`📊 Exporting training data for ${userId ? '1 user' : 'all users'}`);
        ...
        const exportData = {
            ...
            stats: {
                ...
                averageInstructionLength: Math.round(result.trainingPairs.reduce((sum, p) => sum + p.instruction.length, 0) / result.trainingPairs.length),
                averageResponseLength: Math.round(result.trainingPairs.reduce((sum, p) => sum + p.response.length, 0) /
        ...
        console.log(`✅ Exported ${result.trainingPairs.length} training pairs for global fine-tuning`);
        console.log(`📈 Data spans ${uniqueUsers.length} users and ${uniqueRepos.length} repositories`);
        res.json(exportData);
    }
    catch (error) {
...
export const triggerTraining = async (req, res) => {
    try {
        ...
        const response = {
            ...
            manualCommands: {
                export: `curl -X GET "${req.protocol}://${req.get('host')}/api/training/export-data?min_pairs=${minPairs}" > training_data_${jobId}.json`,
 ...
 */
export const startTrainingRun = async (req, res) => {
    try {
        const minPairs = req.body.min_pairs || 200;
        ...
        const lastDataTimestamp = lastTrainingRun?.lastDataTimestamp || new Date('2020-01-01');
        console.log(`📊 Checking for new data since last training run: ${lastDataTimestamp}`);
        ...
        if (result.pairCount < minPairs) {
            res.status(400).json({
                ...
                lastTrainingRun: lastTrainingRun
                    ...
                    : null,
                recommendation: 'Wait for more user interactions or lower min_pairs threshold',
            });
            return;
        }
        ...
        console.log(`🚀 Training run started: ${runId} with ${result.pairCount} new pairs`);
        const response = {
            runId,
            modelVersion,
            status: 'started',
            ...
            manualCommands: {
                export: `curl -H "Authorization: Bearer TEAM_TOKEN" "${req.protocol}://${req.get('host')}/api/training/export-data?min_pairs=${minPairs}&since_last_run=true" > training_data_${runId}.json`,
 ...
 */
export const completeTrainingRun = async (req, res) => {
    try {
        ...
        const duration = Math.round((completedAt.getTime() - trainingRun.startedAt.getTime()) / 60000); // minutes
        ...
        await trainingRun.save();
        console.log(`${success ? '✅' : '❌'} Training run ${success ? 'completed' : 'failed'}: ${runId} (${duration}m)`);
        res.json({
            runId,
