...
export const handleGitHubCallback = async (req, res) => {
    try {
        ...
        console.log('🚀 Redirecting to:', redirectUrl, 'at:', new Date().toISOString());
        if (isInstalled) {
...
export const getGitHubUserOrgs = async (req, res) => {
    ...
    try {
        ...
        let githubToken = authHeader?.startsWith('Bearer ')
            ...
            : cookieToken;
        if (!githubToken) {
            console.error('❌ getGitHubUserOrgs: No GitHub token found in header or cookies', {
        ...
        console.log('🔐 Using GitHub token from:', authHeader ? 'Authorization header' : 'cookie', {
        ...
        if (cached && Date.now() - cached.timestamp < CACHE_TTL) {
            console.log(`📦 [ORGS] Returning cached organizations (${Date.now() - startTime}ms)`);
        ...
        if (inFlightRequests.has(requestKey)) {
            ...
            console.log(`✅ [ORGS] Got result from existing request (${Date.now() - startTime}ms)`);
            res.json(result);
            return;
        }
        ...
        const fetchPromise = (async () => {
            ...
            console.log(`📡 [ORGS] GitHub API responded in ${Date.now() - apiStartTime}ms`);
            if (!orgsResponse.ok) {
        ...
        console.error(`❌ [ORGS] Error after ${Date.now() - startTime}ms:`, err.message);
...
export const getGitHubRepoContents = async (req, res) => {
    const githubToken = req.headers.authorization?.startsWith('Bearer ')
        ? req.headers.authorization.slice(7)
        : req.cookies.github_access_token;
    if (!githubToken) {
