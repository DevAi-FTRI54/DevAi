...
export async function getAppInstallations(accessToken) {
    ...
    if (!response.ok) {
        throw new GitHubApiError('Failed to fetch app installations', response.status, data);
    }
    return data;
}
// Check if GitHub App is installed
export function checkIfAppInstalled(installations) {
    if (!installations?.installations?.length) {
        return { isInstalled: false };
    }
    const appInstallation = installations.installations.find((inst) => inst.app_id === APP_ID || inst.app_slug === APP_SLUG);
    return {
