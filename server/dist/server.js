...
function startServer() {
    const server = app.listen(Number(port), '0.0.0.0', () => {
        ...
        console.log(`🏥 Health / keep-alive: http://localhost:${port}/api/health and /api/keep-alive`);
    });
    server.on('error', (err) => {
    ...
    connectMongo()
        ...
        .catch((e) => console.error('⚠️ MongoDB connection failed (server will continue):', e));
    import('./features/indexing/vector.service.js')
        .then((m) => m.ensureQdrantIndexes())
        .then(() => console.log('✅ Qdrant indexes ready'))
        .catch((e) => console.error('⚠️ Qdrant setup failed (server will continue):', e));
    import('./features/indexing/index.job.js')
        .then(() => console.log('✅ Index job worker loaded'))
        .catch((e) => console.error('⚠️ Index job worker failed to load (jobs will not run):', e));
}
startServer();
